...
public class ParcelController {
    ...
    @GetMapping("/getallbyname/{username}")
    public ResponseEntity<List<Parcel>> getAllParcelsByUsername(@PathVariable String username){
    	
    	List<Parcel> plist= parcelService.getParcelByUsename(username);
    	if(plist.isEmpty()) {
    		return new ResponseEntity<List<Parcel>>(HttpStatus.NOT_FOUND);
    	}
    	else {
    		return new ResponseEntity<List<Parcel>>(plist,HttpStatus.OK);
    	}
    }
